...
if(isset($_POST['page']))
{
	...
	if($_POST['page'] == 'exam')
	{
		...
		{
			// Prepare DataTables parameters safely
			$output = array();
			...
			$order_dir = isset($_POST['order'][0]['dir']) && in_array(strtoupper($_POST['order'][0]['dir']), array('ASC','DESC')) ? strtoupper($_POST['order'][0]['dir']) : 'DESC';
			$start = isset($_POST['start']) ? (int) $_POST['start'] : 0;
			$length = isset($_POST['length']) ? (int) $_POST['length'] : 10;
			$draw = isset($_POST['draw']) ? intval($_POST['draw']) : 0;

			// Base query for this admin
			$exam->query = "SELECT * FROM online_exam_table WHERE admin_id = '" . $_SESSION['admin_id'] . "' ";

			// Add search filters only when provided
			if($search_value !== '')
			{
				$sv = $search_value;
			...
			// ORDER BY mapping (whitelist columns)
			$columns = array(
				0 => 'online_exam_title',
			...
			// Save filtered rows count before limit
			$filtered_rows = $exam->total_row();

			// Add limit
			if($length != -1)
			{
				$exam->query .= ' LIMIT ' . $start . ', ' . $length;
			}
